...
public class PAA extends SimpleBatchFilter {
	...
	private double[] convertInstance(double[] data) throws Exception {
		// added for uneven length by suhita
		for (int i = 0; i < data.length; i++) {

		}
		if (numIntervals > data.length)
			throw new Exception("Error converting to PAA, number of intervals (" + numIntervals + ") greater"
	...
	public static void main(String[] args) {
		...
		try {
			Instances test = ClassifierTools.loadData("C:/Users/com/Dropbox/Content/Tune2/ms1/nikhil.arff");
			PAA paa = new PAA();
			paa.setNumIntervals(1918);
			...
			saver.setFile(new File("C:/Users/com/Dropbox/Content/Tune2/ms1/nikhil_paa.arff"));
			saver.writeBatch();
		} catch (Exception e) {
		...
		// Jason's Test
		try {
			double[] wavey = { 0.841470985, 0.948984619, 0.997494987, 0.983985947, 0.909297427, 0.778073197,
					0.598472144, 0.381660992, 0.141120008, -0.108195135, -0.350783228, -0.571561319, -0.756802495,
					-0.894989358, -0.977530118, -0.999292789, -0.958924275, -0.858934493, -0.705540326, -0.508279077,
					-0.279415498 };
