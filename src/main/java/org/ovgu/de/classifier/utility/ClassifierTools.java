 ...
 * @author ajb
 *
 *         Methods to perform Classification tasks with Weka which I cant seem
 *         to do in Weka
 */
public class ClassifierTools {

	private static Logger logger = Logger.getLogger("ClassifierTools");
	...
	public static Instances loadData(String fullPath) {
		...
		// added by JAL, saves having to manually trim .arff from filename if it's read
		// in from somewhere/using old code
	...
	// e.g. looping though transformed data - if only partial results exist, we can
	// just catch and skip missing datasets rather than nuking the whole VM
	 ...
	 * 
	 * @param file
	 *            the File pointer rather than the path. Useful if you use
	 *            FilenameFilters.
	...
	/**
	 * This method does a cross validation using the EvaluationUtils and stores the
	 * predicted and actual values. I implemented this because I saw no way of using
	 * the built in cross vals to get the actual predictions, useful for e.g
	 * McNemar's test (and cv variance). Note that the use of FastVector has been
	 * depreciated
	 * 
	 * @param c
	 * @param allData
	...
	public static double[][] crossValidation(Classifier c, Instances allData, int m) {
		...
		try {
			...
			evalU.setSeed(10);
			f = (FastVector) evalU.getCVPredictions(c, allData, m);
			p = f.toArray();
	...
	/**
	 * This method does a cross validation using the EvaluationUtils and stores t he
	 * predicted and actual values. Accuracy is stored in preds[0][0], StdDev of
	 * accuracy between folds SHOULD BE stored in preds[1][0]. TO IMPLEMENT! Could
	 * do with some testing, there is some uncertainty over the last fold.
	 * 
	 * @param allData
	 * @param m
	 * @return
	...
	public static double stratifiedCrossValidation(Instances data, Classifier c, int folds, int seed) {
		...
		for (int n = 0; n < folds; n++) {
			...
			try {
				...
				// System.out.println("Finished fold "+n+" acc
				// ="+((double)correct/((n+1)*test.numInstances())));
			} catch (Exception e) {
	...
	/**
	 * This does a manual cross validation (i.e. without EvalUtils) and rather
	 * confusingly returns the distribution for each Instance (as opposed to the
	 * predicted/actual in method performCrossValidation
	 * 
	 * @param data
	 * @param c
	 * @param numFolds
	...
	public static ClassifierResults constructClassifierResults(Classifier classifier, Instances test) throws Exception {
		...
		ClassifierResults results = new ClassifierResults(accuracy, classVals, preds, distForInstances,
				test.numClasses());
	...
	public static class ResultsStats {
		...
		public void findCVMeanSD(double[][] preds, int folds) {
			...
			// System.out.println("Window = "+window+" readings; excess goes in last fold");
			// // Test output
			...
			// System.out.println("Final fold has accuracy = " + acc[folds-1]);//Test
			// outputs
			...
			for (int i = 0; i < folds - 1; i++)// Changed from int i=1
			{
				...
				// System.out.println("Accuracy used here = " +acc[i]); //Test output, added
				// braces.
			}
	...
	// If the folds is 1, do a simple test/train split. Otherwise, do a cross
	// validation by first combining the sets
	public static ResultsStats[] evalClassifiers(Instances test, Instances train, int folds, Classifier[] sc)
			throws Exception {
	...
	public static Instances estimateMissing(Instances data) {
		...
		try {
			...
			// System.out.println(" Filtered? num atts = "+nd.numAttributes()+" num inst =
			// "+nd.numInstances()+" filter = "+nb);
			if (nb.batchFinished())
	 ...
	 * 
	 * NOTE dummy created for all values, so the matrix is not full rank If a
	 * regression formulation required (e.g. 6 binarys for 7 attribute values) call
	 * makeBinaryFullRank
	 * 
	 * @param data
	 */
