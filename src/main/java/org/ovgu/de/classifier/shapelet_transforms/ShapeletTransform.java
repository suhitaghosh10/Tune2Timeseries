...
import org.ovgu.de.classifier.shapelet_transforms.quality_measures.ShapeletQuality;
import org.ovgu.de.classifier.shapelet_transforms.quality_measures.ShapeletQuality.ShapeletQualityChoice;
import org.ovgu.de.classifier.shapelet_transforms.search_functions.FastShapeletSearch;
...
public class ShapeletTransform extends SimpleBatchFilter implements SaveParameterInfo{
    ...
    public Instances process(Instances data) throws IllegalArgumentException {
        ...
        if (!m_FirstBatchDone) {
        	//This is changing the no of shapelets from 272 to 0. Why?
            trainShapelets(data);
    ...
    protected Shapelet checkCandidate(double[] series, int start, int length) {
        ...
        for (int i = 0; i < dataSize; i++) {
            ...
            if (i != dataSet) {
                //Remove "?" from existing time series before computing distances.
                 int j=0;
                 double[] currentSeries=inputData.instance(i).toDoubleArray();
             for( int x=0; x<currentSeries.length; x++ ){
                 if (!Double.isNaN(currentSeries[x]))
                         currentSeries[j++] = currentSeries[x];
             }
             double[] questionMarkRemovedSeries = new double[j];
             System.arraycopy( currentSeries, 0, questionMarkRemovedSeries, 0, j );
//           System.out.println("I am printing the array"+Arrays.toString(questionMarkRemovedSeries));
//           distance = subseqDistance.calculate(inputData.instance(i).toDoubleArray(), i);
             distance = subseqDistance.calculate(questionMarkRemovedSeries, i);
            }
        ...
        Shapelet shapelet = new Shapelet(candidate, dataSourceIDs[dataSet], start, quality.getQualityMeasure());
    ...
    public static ArrayList<Shapelet> readShapeletCSV(File f) throws FileNotFoundException{
        ...
        while(sc.hasNextLine()){
            ...
            if(readHeader){
                ...
                shapelets.add(new Shapelet(content, quality, series, position));   
            }
    ...
    public static void main(String[] args){
        try {
            ...
            test = org.ovgu.de.classifier.utility.ClassifierTools.loadData(filePath + "_TEST");
            train = org.ovgu.de.classifier.utility.ClassifierTools.loadData(filePath + "_TRAIN");
